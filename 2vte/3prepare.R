...
data <- read.csv("df_VTE.csv", header = TRUE, row.names = 1)

table(data$vte_class)

data$vte_class[data$vte_class %in% c("DVT only", "PE + DVT", "PE only")] <- "Yes"
data$vte_class[data$vte_class == "No VTE"] <- "No"
...
names(data)[1] <- "VTE"
data$VTE <- factor(data$VTE, levels = c("No", "Yes"))
...
sele_variables <- sele_variables$Variable

sele_variables <- c("VTE", sele_variables)
print(sele_variables)

# Keep only VTE and modeling variables here; VTE_clinical is not used for modeling
...
feat_base <- feat_raw %>%
  ...
  stringr::str_replace("^who_grade.*", "who_grade") %>%
  stringr::str_replace("^pathology.*", "pathology") %>%
...
df_top5_train <- train_data %>% dplyr::select(VTE, all_of(top5_vars))
df_top5_test  <- test_data  %>% dplyr::select(VTE, all_of(top5_vars))

df_top10_train <- train_data %>% dplyr::select(VTE, all_of(top10_vars))
df_top10_test  <- test_data  %>% dplyr::select(VTE, all_of(top10_vars))

df_top13_train <- train_data %>% dplyr::select(VTE, all_of(top13_vars))
df_top13_test  <- test_data  %>% dplyr::select(VTE, all_of(top13_vars))

df_caprini_train <- train_data %>% dplyr::select(VTE, pre_caprini_score)
df_caprini_test  <- test_data  %>% dplyr::select(VTE, pre_caprini_score)
...
write.csv(df_top13_train,   file = "df_top13_train.csv",   row.names = TRUE)
write.csv(df_top13_test,    file = "df_top13_test.csv",    row.names = TRUE)

write.csv(df_caprini_train, file = "df_caprini_train.csv", row.names = TRUE)
write.csv(df_caprini_test,  file = "df_caprini_test.csv",  row.names = TRUE)
