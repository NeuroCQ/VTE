...
convert_binary_cols <- function(df,
  ...
  for (v in cols) {
    ...
    if (label_01 && all(ux_chr %in% c("0","1"))) {
      df[[v]] <- factor(as.character(x),
                        levels = c("0","1"),
                        labels = c("No","Yes"))
      next
    }
    
    # Yes/No/True/False etc. -> standardize to No/Yes
    ...
    # Any other two-class values: convert to factor and keep original values (stable ordering)
...
set.seed(5)
train_index <- createDataPartition(y = data_model$VTE, p = 0.8, list = FALSE)
...
# Standardize binary columns except VTE and dataset
data_model <- convert_binary_cols(
  data_model,
  exclude = c("VTE", "dataset"),
  label_01 = TRUE
)
...
tbl_train_test <- data_model %>%
  tbl_summary(
    by = dataset,
    label = as.list(feature_labels[names(feature_labels) %in% names(data_model)]),
